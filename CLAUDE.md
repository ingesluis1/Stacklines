...
- **Flow-rail (`.flow-rail`) op mobiel**: de sticky sectie-rail (oranje bolletjes + labels) staat op desktop links en toont labels op hover/actief. Op **<1100px** staat 'ie **rechts** (`right`, `left:auto`) en toont **alleen de bolletjes**: labels zijn daar standaard verborgen (óók voor de actieve sectie) en verschijnen als **tooltip-pill links van het bolletje zolang je het ingedrukt houdt** (`.holding`-class via `pointerdown`/`pointerup` in `script.js` sectie 4, plus `:active` als fallback; loslaten/wegscrollen = label weg). De labels staan `position:absolute` zodat alleen de dots een smalle kolom vormen en de progress-track ermee uitgelijnd blijft. Loslaten navigeert wel gewoon naar de sectie (het blijven nav-links). **iOS-fix:** `.flow-rail a` heeft `-webkit-touch-callout: none` + `user-select: none`, anders opent Safari bij ingedrukt houden z'n eigen link-preview/-menu i.p.v. ons label.
