...
- **Slang op de achtergrond (`.bg-snake` + `.snake-move`/`.snake-line`)**: vervangt op mobiel de desktop-slingers. **Twee evenwijdige golflijnen in dezelfde fase** (`.s1` oranje, `.s2` peach, `.s2` staat 6 eenheden naast `.s1` zodat ze samen meegolven i.p.v. te kruisen, geen DNA-effect), in dezelfde zachte stijl/palet als de desktop-slingers (`snakeBreathe1`/`snakeBreathe2`; de -3s op `snakeBreathe2` is alleen een kleur-pulse-offset, niet de vorm). **Vloeiend, geen harde hoeken**: de bezier-controlepunten zijn per knooppunt gespiegeld (in/uit-handle collineair door het anker), dus de raaklijn loopt continu door. De slang **scrollt NATIEF met de pagina mee** i.p.v. via een scroll-gedreven transform. De laag is `.bg-snake { position: absolute }` over de **volle documenthoogte** (JS zet `height` = `document.documentElement.scrollHeight`), dus de browser verschuift 'm zelf, exact 1:1 strak en zonder lag. *Historie:* eerst was 'ie `position: fixed` met een JS-transform op `scrollY` (eerst hard gepind = schokkerig, daarna een lerp = voelde "los"/liep íets achter). Beide hadden last van het main-thread/compositor-verschil; natief scrollen lost dat fundamenteel op. **JS (`script.js` sectie 7) doet tijdens scrollen NIKS** — het berekent alleen bij load/resize/reflow (`ResizeObserver` op `body`) de `viewBox`-hoogte (`200*docH/vh`, zodat 100 units = vh/2 px → zelfde verticale periode als vroeger) en bouwt beide pad-`d`'s opnieuw op via `buildWave(cx, vbh)` (amp 40 = horizontale uitslag, control-y ±14 voor vloeiende raaklijnen). De `d` in de HTML is enkel een fallback vóór JS draait. **Trade-off:** de oude viewport-mask ("uitvloeien onderin") is vervallen, want dat is een per-viewport-effect dat niet samengaat met natief meescrollen. Over de donkere contact-sectie is de slang sowieso afgedekt. Bij `prefers-reduced-motion` staat alleen de kleur-ademing stil; de positie volgt gewoon de scroll. Tunebaar in `buildWave`: horizontale uitslag via `amp` (nu 40, midden-x 50 en 56), rondheid via de control-y-offset `cy` (nu ±14 rond elk anker)
