...
- **Slang op de achtergrond (`.bg-snake` + `.snake-move`/`.snake-line`)**: vervangt op mobiel de desktop-slingers. **Twee evenwijdige golflijnen in dezelfde fase** (`.s1` oranje, `.s2` peach, `.s2` staat 6 eenheden naast `.s1` zodat ze samen meegolven i.p.v. te kruisen, geen DNA-effect), in dezelfde zachte stijl/palet als de desktop-slingers (`snakeBreathe1`/`snakeBreathe2`; de -3s op `snakeBreathe2` is alleen een kleur-pulse-offset, niet de vorm). **Vloeiend, geen harde hoeken**: de bezier-controlepunten zijn per knooppunt gespiegeld (in/uit-handle collineair door het anker), dus de raaklijn loopt continu door. De slang **kruipt continu** via een CSS-transform-animatie (`@keyframes snakeCrawl` op `.snake-move`, `9s linear infinite`), los van de scroll. De laag is `.bg-snake { position: fixed }` met een `mask-image` die de golven onderin laat uitvloeien (`#000 0%, #000 58%, transparent 70%`) — die uitvloei-fade verkoopt de kruip-illusie. De animatie translateY't `.snake-move` over precies **één golfperiode** (100 user-units = vh/2 op scherm; periode 100 in y), dus de sprong terug naar 0 is onzichtbaar = naadloze loop. Draait op de compositor → soepel, geen JS, geen lag of schokken. **Belangrijk inzicht uit de iteratie:** een *kruipende* slang moet bewegen t.o.v. de pagina; "strak 1:1 meescrollen" (vastplakken aan de content) dóódt juist de kruip. *Historie:* geprobeerd: `position: fixed` + JS-transform op `scrollY` (hard gepind = schokkerig; lerp = voelde "los"), en natief meescrollen via `position: absolute` over de volle documenthoogte (exact strak, maar geen kruip + fade weg = illusie weg). Uiteindelijk gekozen voor continue CSS-kruip + fade terug. JS (`script.js` sectie 7) doet hier niks meer. Bij `prefers-reduced-motion` staan zowel de kruip-animatie (`.snake-move`) als de kleur-ademing (`.snake-line`) stil. Tunebaar: kruipsnelheid via de animatieduur (nu `9s`, korter = sneller), golfvorm via de bezier-paden in de HTML (ankers op x=50/56, control-x ±40 voor de uitslag, control-y ±14 voor de rondheid)
