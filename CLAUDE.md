...
- **Hero-visual = de postvak-assistent-demo** (rechterkolom, sinds 2026-06-20). De hero is nu twee kolommen: tekst links, een vanilla chat-mockup rechts die een vraag typt, "denkt" en een antwoordkaart toont (`.sl-assistant`, markup-skelet in hero, styling in style.css onder "Postvak-assistent", gedrag in script.js sectie 8). **De demo loopt door meerdere scenario's** (`SCENARIOS`-array in script.js sectie 8): mail zoeken, mails samenvatten, concept-antwoord schrijven, openstaande facturen, afspraak inplannen. Elk scenario = `{query, result}`; antwoordkaarten worden via de `reply()`-helper gewrapt en hergebruiken grotendeels de `.sl-email__*`-classes plus een paar eigen (`.sl-sum`, `.sl-flag`, `.sl-draft__*`, `.sl-rows`/`.sl-row`/`.sl-total`, `.sl-slot__*`). **Het venster heeft een vaste hoogte** (`.sl-assistant__body { min-height: 440px }`) zodat het niet groeit/krimpt tussen staten; de mailkaart is de hoogste staat, houd nieuwe kaarten daaronder. Voorbeeldnamen (Van der Berg, Jansen Advies, De Wit Bouw) zijn fictief en generiek. Eigen `--sl-*` tokens gescoped op `.sl-assistant`, palet matcht de site, font erft Inter. Twee-koloms breekpunt staat bewust op **901px** (= waar scroll-snap aangaat) zodat er geen tussenzone is waar de hero gestapeld binnen 100vh afsnijdt. Op mobiel (≤900px) stapelt de demo gecentreerd onder de tekst. *Historie:* eerder was de hero bewust minimaal (alleen tekst + slingers); een uitgebreide workflow/n8n-tekening was geprobeerd en verwijderd (te druk). De assistent-demo is na overleg met de user toegevoegd omdat 'ie inhoudelijk (mail zoeken/samenvatten) precies het AI-verhaal toont. Op mobiel komt i.p.v. de slingers een aparte "slang" terug (zie sectie Mobiel)
